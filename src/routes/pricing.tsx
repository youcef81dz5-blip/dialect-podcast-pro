  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-4xl px-6 pb-24 pt-8">
        <h1 className="text-center text-3xl font-bold md:text-4xl">{t("خطط بسيطة حسب دقائقك")}</h1>
        <p className="mx-auto mt-4 max-w-xl text-center text-muted-foreground">
          {t("ابدأ مجاناً بـ 30 دقيقة شهرياً، وارفع الحد حين ينمو إنتاجك. يتجدد الرصيد تلقائياً كل شهر.")}
        </p>
            ...
            >
              {plan.highlight && (
                <span className="inline-flex rounded-full bg-primary/15 px-3 py-1 text-xs font-medium text-primary">
                  {t("الأكثر ملاءمة للبودكاسترز")}
                </span>
              )}
              <h2 className="mt-4 text-xl font-bold">{t(plan.name)}</h2>
              <p className="mt-3 text-3xl font-bold">
                {plan.price}{" "}
                <span className="text-sm font-normal text-muted-foreground">/ {t(plan.period)}</span>
              </p>
              ...
              >
                {plan.highlight ? (
                  <span>{t("الترقية قريباً")}</span>
                ) : (
                  <Link to={user ? "/app" : "/auth"}>{t("ابدأ الآن مجاناً")}</Link>
                )}
              </Button>
            </div>
          ))}
        ...
        <p className="mt-10 text-center text-xs text-muted-foreground">
          {t("الدفع الإلكتروني قيد الإعداد؛ حتى ذلك الحين كل الحسابات على الخطة المجانية بـ 30 دقيقة شهرياً.")}
        </p>
      </main>
    </div>
  );
