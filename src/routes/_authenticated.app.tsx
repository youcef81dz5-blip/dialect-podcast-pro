  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">{t("حلقاتي")}</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {t("ارفع حلقة لتبدأ التفريغ والترجمة.")}
          </p>
        </div>
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-3">
        <div className="rounded-2xl border bg-card p-6 md:col-span-1">
          ...
          <p className="mt-3 text-2xl font-bold">
            ...
            <span className="text-sm font-normal text-muted-foreground">/ {quota}</span>
          </p>
          <Progress value={pct} className="mt-4" />
          <p className="mt-3 text-xs text-muted-foreground">
            {t("الخطة الحالية:")} {subscription?.plan === "pro" ? t("احترافي") : t("مجاني")}
          </p>
          {quota - used <= 0 && (
